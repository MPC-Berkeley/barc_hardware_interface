...
// >= BARC_Nano_Shield_v3
#define THROTTLE_PIN D3
#define STEERING_PIN D4
...
void setup_actuators(){
  ...
  brushless_motor.attach(THROTTLE_PIN);
}

void reset_actuators(){
  steering_servo.writeMicroseconds(STEERING_OFF);
  brushless_motor.writeMicroseconds(THROTTLE_OFF);
  delay(3000);
  update_actuators();
}
...
void update_actuators(){
  ...
  if (output_enabled){
    ...
    brushless_motor.writeMicroseconds(throttle_v);
...
void write_throttle(unsigned int m){
  throttle_v = min(max(m, THROTTLE_MSG_MIN), THROTTLE_MSG_MAX);
  if (DEBUG) {
